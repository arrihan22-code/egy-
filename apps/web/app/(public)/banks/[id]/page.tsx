...
export default function BankDetailPage() {
  ...
  return (
    <PageSection>
      <BackButton href="/banks" labelKey="Back to Banks" labelAr="العودة إلى البنوك" />

      <div className="slide-up">
        <div style={{ background: 'var(--surface-elevated)', border: '1px solid var(--border)', borderRadius: 'var(--radius-2xl)', padding: 'var(--space-8)', marginBottom: 'var(--space-6)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-4)', flexWrap: 'wrap' }}>
            <div style={{ width: 56, height: 56, borderRadius: 'var(--radius-xl)', background: 'var(--primary-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', flexShrink: 0 }}>
              {'\u{1F3E6}'}
            </div>
            <div style={{ flex: 1 }}>
        ...
        {markers.length > 0 && (
          <div style={{ marginBottom: 'var(--space-6)' }}>
            <h2 style={{ fontSize: 'var(--text-xl)', fontWeight: 600, marginBottom: 'var(--space-4)' }}>{t('Locations', 'المواقع')}</h2>
              ...
              {bank.branches.map((branch: BankBranch) => (
                <div key={branch.id} className="card" style={{ border: 'none', background: 'var(--surface-secondary)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                    {branch.hasAtm && <Badge variant="primary">{t('ATM', 'صراف آلي')}</Badge>}
                  </div>
                </div>
              ))}
            </div>
